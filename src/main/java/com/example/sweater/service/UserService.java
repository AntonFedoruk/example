...
public class UserService implements UserDetailsService {
    ...
    public boolean addUser(User user) {
        ...
        user.setPassword(passwordEncoder.encode(user.getPassword()));//use PasswordEncoder to encode user`s password
    ...
    private void sendMessage(User user) {
        ...
        if (!StringUtils.isEmpty(user.getEmail())) {
            String message = String.format(
                    "Hello, %s! \n" +
                            "Welcome to Sweater. Please, visit next link: http://%s/activate/%s", /* in future link could be replaced to .property */
                    user.getUsername(),
                    hostname,
